...
static uint16_t accel_data_handle;
...
static int
gatt_svr_chr_access_accel_data(uint16_t conn_handle,
...
//            .access_cb = gatt_svr_chr_access_sc_control_point,
  //          .val_handle = &csc_control_point_handle,
    //        .flags = BLE_GATT_CHR_F_WRITE | BLE_GATT_CHR_F_INDICATE,
        ...
        .type = BLE_GATT_SVC_TYPE_PRIMARY,
        .uuid = BLE_UUID16_DECLARE(0x1234),  // Custom UUID for accelerometer service
        .characteristics = (struct ble_gatt_chr_def[]) { {
            /* Characteristic: Accelerometer Data */
            .uuid = BLE_UUID16_DECLARE(0x5678),  // Custom UUID for accelerometer characteristic
            .access_cb = gatt_svr_chr_access_accel_data,
            .val_handle = &accel_data_handle,
            .flags = BLE_GATT_CHR_F_NOTIFY,
        }, {
...
static int
gatt_svr_chr_access_accel_data(uint16_t conn_handle, uint16_t attr_handle,
                                struct ble_gatt_access_ctxt *ctxt, void *arg)
 {
    //icm42670_value_t imu_data;
    

    // Prepare the accelerometer data to be sent
    uint8_t accel_data[6] = {
        ((int)(1000*imu_data.x) & 0xFF), ((int)(1000*imu_data.x) >> 8),
        ((int)(1000*imu_data.y) & 0xFF), ((int)(1000*imu_data.y) >> 8),
       ((int)(1000*imu_data.z) & 0xFF), ((int)(1000*imu_data.z) >> 8)
    };

    // Send the accelerometer data to the client
    int rc = os_mbuf_append(ctxt->om, accel_data, sizeof(accel_data));
